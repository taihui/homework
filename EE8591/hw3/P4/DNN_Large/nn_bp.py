...
def deepreg_train(learning_rate,max_epoch,train_batch_size,val_batch_size,test_batch_size,gpu,print_step,opm_tag,train_f5,train_f8,val_f5,val_f8,test_f5,test_f8):
    ...
    for epoch in range(max_epoch):
        ...
        for step, (X_data, Y_labels, idx) in enumerate(train_loader):
            ...
            Y_labels = Y_labels.to(device).flatten().float()
            # zero the parameter gradients
            optimizer.zero_grad()
            # forward + backward + optimize
            Y_pred = DLRegNet(X_data).flatten().float()
    ...
    f.write('final_train_acc={}'.format(final_train_acc))
    f.write('final_val_acc={}'.format(final_val_acc))
    f.write('final_test_acc={}'.format(final_test_acc))
    f.close()
    print("Finish!")
...
def deep_reg_val(model, data_loader, device):
    ...
    with torch.no_grad():
        for step, (X_data, Y_label, idx) in enumerate(data_loader):
            X_data = X_data.to(device).float()
            Y_label = Y_label.to(device).flatten().float()
            Y_pred = model(X_data).flatten().float().detach()
